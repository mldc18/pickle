-- Keep capacity snapshots as historical records only. Today and future dates
-- should follow the current default capacity unless an admin sets an override.

update public.game_days
set capacity_snapshot = null
    ...
    least(
      ...
      coalesce(
        ...
        (
          ...
          from public.game_days
          where game_days.date = p_date
            and p_date < (now() at time zone 'Asia/Manila')::date
        ),
        (
